...
Two tiers:
  ...
  2. ChromaDB vector search — optional, activated when ``chromadb`` is
     installed and an index has been built via ``logatory llm index``.
...
def _chroma_search(
    ...
    try:
        col = client.get_collection("logatory")
    except Exception:
        return []
...
def build_chroma_index(db_path: Path, chroma_path: Path, client) -> int:
    ...
    try:
        chroma.delete_collection("logatory")
    except Exception:
        pass
    col = chroma.create_collection("logatory")
