...
""",
    ...
    typer.echo("  Custom rules: logatory rules validate my_rule.yml")
    typer.echo("  List rules  : logatory rules list")
    _pause(no_pause)

    # ── 4. Error Tracking ──────────────────────────────────────────────────
    _h("4 / 7  -- Error Tracking  (--track-errors)")
    typer.echo("\n  Errors are deduplicated by fingerprint and persisted to SQLite.\n")
    typer.echo("  $ logatory errors list\n")
    ...
    typer.echo("\n  Extras: logatory errors show <fp>  |  logatory errors new --since 24h")
    typer.echo("          logatory errors regression  (errors that reappeared after silence)")
    _pause(no_pause)
    ...
    typer.echo("  $ logatory findings list\n")
    ...
    typer.echo("  $ logatory scan app.log --detect-anomalies\n")
    ...
    typer.echo("\n  Training:  logatory anomaly learn app.log")
    typer.echo("  Status:    logatory anomaly status")
    typer.echo("  Reset:     logatory anomaly reset --source app.log")
    _pause(no_pause)
  ...
  Quick-start:
    ...
    logatory scan /var/log/syslog --track-errors --detect-anomalies
    logatory errors list
    logatory findings list
    logatory llm info
    logatory llm explain <fingerprint>
    logatory llm ask "what happened last night?"
  ...
  Run this demo again:  logatory demo run
  Run without pauses:   logatory demo run --no-pause
""")
...
def _load_cfg(config: Path | None):
    """Load config, falling back to the LOGATORY_CONFIG env var if set."""
    import os

    from logatory.config import Config

    if config is None:
        env_path = os.environ.get("LOGATORY_CONFIG", "")
        if env_path:
...
def demo_seed(
    ...
    with 'logatory demo clear' without affecting real data.
    """
    import json
    import sqlite3

    from logatory.storage.errors_schema import ERRORS_SCHEMA_SQL
    ...
    if findings_inserted == 0 and errors_inserted == 0:
        typer.echo(
            typer.style(
                "  Demo data already present. Run 'logatory demo clear' first to reseed.",
    ...
    typer.echo("\n  Open the dashboard:  logatory serve")
    typer.echo("  Remove demo data:    logatory demo clear")
    typer.echo(f"  {sep}\n")
...
def demo_clear(
    ...
    Only records tagged by 'logatory demo seed' are deleted.
